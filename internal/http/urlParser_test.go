...
func TestLenUrl(t *testing.T) {
	_, err := http.ParseURL(&url.URL{Path: "/fill/300/200/site.com/image.jpeg"})
	if err != nil {
		t.Error(err)
	}

	_, err = http.ParseURL(&url.URL{Path: "/fill/300/200/site.com"})
	if err == nil {
...
func TestServiceType(t *testing.T) {
	_, err := http.ParseURL(&url.URL{Path: "/some/300/200/site.com/image.jpeg"})
	if err == nil {
		t.Error("no error on wrong url service")
	}

	urlParams, err := http.ParseURL(&url.URL{Path: "/fill/300/200/site.com/image.jpeg"})
	if err != nil || urlParams.Service != "fill" {
		t.Error("error on correct url service fill ")
	}

	urlParams, err = http.ParseURL(&url.URL{Path: "/resize/300/200/site.com/image.jpeg"})
	if err != nil || urlParams.Service != "resize" {
		t.Error("error on correct url service resize")
	}

	urlParams, err = http.ParseURL(&url.URL{Path: "/fit/300/200/site.com/image.jpeg"})
